...
function configurarAbas() {
  document.querySelectorAll('.aba').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      // 🆕 clicar num botão de ranking (Diário/Dia Anterior/Semanal/Mensal/Ligas/Pontos)
      // agora abre a JANELA do ranking (mesmo visual do Menu de Ajuda) — o conteúdo (busca
      // + lista) é exatamente o mesmo de sempre, só que dentro dela em vez de solto na página
      const modalRanking = document.getElementById('rank-modal-fundo');
      if (modalRanking) modalRanking.classList.add('aberto');
    });
  });
}
